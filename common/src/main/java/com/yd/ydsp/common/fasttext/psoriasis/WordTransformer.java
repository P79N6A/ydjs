...
public class WordTransformer {
    ...
    public WordTransformer(List<String> pinyin, List<String> fork, List<String> deform,
                           List<String> homo) {
     ...
     */
    public WordTransformer(List<String> pinyin, List<String> fork, List<String> deform,
                           List<String> homo, int wordLengthLimit, int wordListLimit) {
    ...
    public List<SkipTermExtraInfo> transformPinyinWords(String word) {
        return transformTransformWords(word, word, c2pTable, 0, pinyinRec,
                SkipTermExtraInfo.PINYIN_TYPE);
    }

    public List<SkipTermExtraInfo> transformForkWords(String word) {
        return transformTransformWords(word, word, c2fTable, 0, forkRec,
                SkipTermExtraInfo.FORK_TYPE);
    }

    public List<SkipTermExtraInfo> transformHomophoneWords(String word) {
        return transformTransformWords(word, word, c2hTable, 0, homoRec,
                SkipTermExtraInfo.HOMOPHONE_TYPE);
    }

    public List<SkipTermExtraInfo> transformDeformWords(String word) {
        return transformTransformWords(word, word, c2dTable, 0, deformRec,
                SkipTermExtraInfo.DEFORM_TYPE);
    }
    ...
    private List<SkipTermExtraInfo> transformTransformWords(String word, String protoWord,
                                                            ...
                                                            int begin, boolean recursive, int type) {
            ...
            if (fork != null && fork.length() != 0) {
                ...
                if (fks != null && fks.length != 0) {
                    for (String fk : fks) {
                        ...
                        String trans = word.substring(0, i) + fk
                                + word.substring(i + 1, word.length());
                        wordList.add(new SkipTermExtraInfo(trans, protoWord, true, false, type));
                        if (recursive && allowRecursive(protoWord, table)) {
                            wordList.addAll(transformTransformWords(trans, protoWord, table, begin
                                    + fk.length(), recursive, type));
                        }
                    }
                }
            }
