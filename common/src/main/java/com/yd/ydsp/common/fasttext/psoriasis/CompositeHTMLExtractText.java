...
import com.yd.ydsp.common.fasttext.extract.CharTableLoader;
...
public class CompositeHTMLExtractText extends HTMLParserExtractText {
    ...
    public CompositeHTMLExtractText(CharTableLoader loader, boolean needHtml, boolean needT2S,
                                    boolean needDBC, boolean ignoreCase,
                                    boolean filterNoneHanLetter, boolean convertSynonymy,
                                    boolean filterSymbol, boolean keepLastSymbol) {
        this(needHtml, needT2S, needDBC, ignoreCase, filterNoneHanLetter, convertSynonymy,
                filterSymbol, keepLastSymbol);
    ...
    public CompositeHTMLExtractText(boolean needHtml, boolean needT2S, boolean needDBC,
                                    boolean ignoreCase, boolean filterNoneHanLetter,
    ...
    public MappedCharArray getText(MappedCharArray src, boolean ignoreCase) {
        ...
        for (int i = 0; i < len; i++) {
            char c = target[i];
            char ret = CharNormalization.compositeCharConvert(c, needT2S, needDBC, ignoreCase,
            ...
            if (ret == CharNormalization.DEFAULT_BLANK_CHAR) {
                if (keepLastSymbol && i < target.length - 1) {
                    ...
                    if (CharNormalization.isSeperatorSymbol(c)
                            && !CharNormalization.isSeperatorSymbol(next)) {
    ...
    public String getText(String html, boolean ignoreCase) {
        ...
        return CharNormalization.compositeTextConvert(text, needT2S, needDBC, ignoreCase,
