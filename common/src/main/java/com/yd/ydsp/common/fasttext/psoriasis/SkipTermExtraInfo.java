...
import com.yd.ydsp.common.fasttext.extract.CharNormalization;
 ...
 * 原词：只匹配词在词典中的值，因为考虑到跳字匹配，部首匹配，tag过滤等情况，例如“法-轮-功”“氵去车仑工力”等词都可以
 * 匹配“法轮功”，word就是“法轮功”，以有利用分词后的后续处理。<br>
 * 权重：有用户设定，代表这个词的重要程度，用于后续的bayes评估算法，在[0,1]之间，以0.5为中间值，
 * 大于0.5会增大整体权重，小于0.5为减小整体权重。<br>
 * 跳字匹配：指定这个词是否可以跳字匹配，如果允许，“法-轮-功""法，轮，功”都可以匹配“法轮功”，否则不可以。<br>
 * 全词匹配：主要用于英文单词，因为darts是基于trie结构，而英文单词往往基于分隔符，例如lee可以匹配“ lee ”和“sleep”中的
 * "lee"，但是显然前者才符合英文习惯。<br>
...
public class SkipTermExtraInfo implements TermExtraInfo, Serializable {
     ...
     * @param wholeWord 主要用于英文单词，因为darts是基于trie结构，而英文单词往往基于分隔符，例如lee可以匹配“ lee
     *            ”和“sleep”中的"lee"，但是显然前者才符合英文习惯。
     * @param weight 
     *            权重：有用户设定，代表这个词的重要程度，用于后续的bayes评估算法，在[0,1]之间，以0.5为中间值，大于0.5会增大整体权重
     *            ，小于0.5为减小整体权重。
     */
     ...
     */
    protected SkipTermExtraInfo(String word, boolean allowSkip, boolean wholeWord, float weight,
                                char folkChar) {
        this.word = word;
    ...
    public SkipTermExtraInfo(String word, String protoWord, boolean allowSkip, boolean wholeword,
                             int transformType) {
        this.word = word;
